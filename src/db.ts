/**
 * SQLite database access layer for the MFSA MCP server.
 *
 * Schema mirrors the MFSA regulatory structure:
 *   - sourcebooks        — top-level MFSA rulebook directories
 *   - provisions         — individual rules, guidance notes, and circulars
 *   - enforcement_actions — MFSA enforcement decisions and penalties
 *
...
export function getProvision(
  ...
  const row = db
    ...
    .get(sourcebook.toUpperCase(), reference.toUpperCase()) as
    | Provision
    | undefined;

  if (row) return row;

  const bare = reference.toUpperCase();
  return (
    (db
      .prepare(
        "SELECT * FROM provisions WHERE sourcebook_id = ? AND reference LIKE ? LIMIT 1",
      )
      .get(sourcebook.toUpperCase(), `${bare}%`) as Provision | undefined) ??
    null
  );
}
